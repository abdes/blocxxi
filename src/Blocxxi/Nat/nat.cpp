...
// TODO: Implement network interface enumeration for Windows
  ...
  LOG_F(1, "enumerating network interfaces and IP addresses");
...
auto GetPortMapper(std::string const &spec) -> std::unique_ptr<PortMapper> {
  LOG_F(1, "NAT port mapper spec: [{}]", spec);
  ...
  if (mechanism == "extip") {
    if (parts.size() == 3) {
      ...
      LOG_F(1, "Missing explicit external IP in 'extip' nat spec");
    }
  ...
  if (mechanism == "pmp") {
    ...
    LOG_F(ERROR, "PMP port mapper is not implemented yet");
    return nullptr;
  }
  ...
  if (IsLocal(address)) {
    ...
    LOG_F(WARNING, "Could not discover external IP address.");
    LOG_F(INFO, "Consider using nat spec 'extip' to manually specify it");
  }
