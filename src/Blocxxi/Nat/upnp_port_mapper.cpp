...
#include <Blocxxi/Nat/error.h>
#include <Blocxxi/Nat/upnp_port_mapper.h>
...
auto UpnpPortMapper::AddMapping(
  ...
  LOG_F(1, "UPNP add mapping ({}/{}) -> ({}, {})", mapping.protocol,
      mapping.external_port, internal_ip_, mapping.internal_port);
  if (urls_->controlURL[0] == '\0') {
    LOG_F(ERROR, "UPNP discovery was not done!");
  ...
  if (failure != 0) {
    ...
    // permanent lease
    LOG_F(ERROR, "UPNP add mapping ({}, {}, {}) failed, code {}",
...
auto UpnpPortMapper::DeleteMapping(PortMapper::Protocol protocol,
  ...
  LOG_F(INFO, "UPNP delete mapping ({}/{})",
...
auto UpnpPortMapper::Discover(std::chrono::milliseconds timeout)
  ...
  LOG_F(1, "starting discovery for UPNP port mapper");
  ...
  if (devlist != nullptr) {
    ...
    if (idg_was_found != 0) {
      LOG_F(1, "UPNP found valid IGD device (status: {}) desc: {}",
      ...
      LOG_F(ERROR, "UPNP failed to obtain external IP");

    } else {
      LOG_F(ERROR, "UPNP no valid IGD was found");
    }
  }
