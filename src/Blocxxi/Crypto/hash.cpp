...
#include <Blocxxi/Crypto/config.h>
...
auto HostToNetwork(std::uint32_t number) -> std::uint32_t
{
  if constexpr (std::endian::native == std::endian::little) {
    return ((number & 0x000000FFU) << 24U) | ((number & 0x0000FF00U) << 8U)
      | ((number & 0x00FF0000U) >> 8U) | ((number & 0xFF000000U) >> 24U);
  }
  return number;
}
  ...
  -> size_t
{
  ...
  for (size_t i = 0; i < num; i++) {
    ...
    constexpr size_t MultiplyDeBruijnBitPosition[32]
      = { 0, 9, 1, 10, 13, 21, 2, 29, 11, 14, 16, 18, 22, 25, 3, 30, 8, 12, 20,
          28, 15, 17, 24, 7, 19, 27, 23, 6, 26, 5, 4, 31 };
