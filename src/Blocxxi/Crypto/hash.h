...
#include <Blocxxi/Codec/base16.h> // for hex enc/decoding
#include <Blocxxi/Crypto/random.h> // for random block generation
...
namespace detail {
  ...
  /// Avoid pulling heavy networking or endian dependencies into this header.
...
template <std::size_t BITS> class Hash {
  static_assert(
    BITS % c_hash_align_at == 0, "Hash size in bits must be a multiple of 32");
...
public:
  ...
  */
    ...
    : storage_({})
  {
    ...
    CHECK_F(src_size <= Size(), "source span exceeds hash size");
    auto start = begin();
    if (Size() > src_size) {
  ...
  {
    CHECK_F(pos < Size(), "hash index out of range");
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    return reinterpret_cast<pointer>(storage_.data())[pos];
  }
  ...
  constexpr static auto Size() -> size_type { return BITS / 8; }

  /// The size of the hash in bytes.
  constexpr static auto BitSize() -> size_type { return BITS; }
  ...
  */
  void Assign(gsl::span<std::uint8_t const> buf, iterator start) noexcept
  {
    ...
    CHECK_F(src_size <= dst_size, "source span exceeds destination range");
