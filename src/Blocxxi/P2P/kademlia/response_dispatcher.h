...
public:
  ...
  //@{
  explicit ResponseDispatcher(asio::io_context& io_context)
    : timer_(io_context)
  {
    LOG_F(1, "Creating ResponseDispatcher DONE");
  }
  ...
  ~ResponseDispatcher() { LOG_F(1, "Destroy ResponseDispatcher"); }
  //@}
    ...
    BufferReader const& buffer)
  {
    ...
    if (failure == UNASSOCIATED_MESSAGE_ID) {
      ...
      LOG_F(1, "dropping response with no registered callback");
    }
  }
  ...
  {
    LOG_F(2, "  register response callback with timeout");
    ...
    timer_.ExpiresFromNow(callback_ttl, [this, on_error, response_id]() {
      ...
      if (RemoveCallback(response_id)) {
        LOG_F(2, "  removed timed out response callback");
        LOG_F(2, "  invoking error handler");
...
private:
    ...
    OnResponseCallbackType const& on_response_received)
  {
    ...
    LOG_F(1, "  add callback to map {}", callbacks_.size());
    ...
    BufferReader const& buffer) -> std::error_code
  {
    ...
    LOG_F(1, "  removed response callback");
