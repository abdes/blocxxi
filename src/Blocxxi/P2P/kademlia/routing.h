 ...
 */
class BLOCXXI_P2P_API RoutingTable {
public:
  /// The logger id used for logging within this class.
  static constexpr const char* LOGGER_NAME = "p2p-kademlia";

  using iterator = std::deque<KBucket>::iterator;
  using const_iterator = std::deque<KBucket>::const_iterator;
  using reverse_iterator = std::deque<KBucket>::reverse_iterator;
  using const_reverse_iterator = std::deque<KBucket>::const_reverse_iterator;
  ...
  /// @name Iteration
  //@{
  auto begin() noexcept -> iterator { return buckets_.begin(); }
  [[nodiscard]] auto begin() const noexcept -> const_iterator
  {
    return buckets_.cbegin();
  }
  auto end() noexcept -> iterator { return buckets_.end(); }
  ...
  [[nodiscard]] auto ThisNode() const -> const Node& { return my_node_; }
