...
class BLOCXXI_P2P_API Timer final {
public:
...
private:
  ...
  /*! The asio::basic_waitable_timer template accepts an optional
   * WaitTraits
  ...
  struct ClockTypeWaitTraits {
      ...
      -> ClockType::duration
    {
      if (duration > std::chrono::seconds(1)) {
        return duration - std::chrono::seconds(1);
      }
  ...
  // using DeadlineTimerType = asio::basic_waitable_timer<ClockType>;
  using DeadlineTimerType
    = asio::basic_waitable_timer<ClockType, ClockTypeWaitTraits>;
  ...
  DurationType const& timeout, Callback const& on_timer_expired)
{
  ...
  LOG_F(1, "adding timer expiring at {}",
