 ...
 */
template <typename TUnderlyingSocket> class Channel final {
public:
   ...
   */
  Channel(asio::io_context& io_context, EndpointType const& endpoint)
    : reception_buffer_(SAFE_PAYLOAD_SIZE)
    , current_message_sender_()
    , socket_(CreateUnderlyingSocket(io_context, endpoint))
  {
    LOG_F(1, "Creating channel {} DONE", endpoint.ToString());
  }
  ...
  ~Channel()
  {
    LOG_F(1, "Destroy channel {}", LocalEndpoint().ToString());
    try {
      socket_.close();
    } catch (std::exception const& ex) {
      // Ignore all errors. The underlying descriptor will be closed anyway.
      LOG_F(1, "an error reported on socket closure: {}", ex.what());
    }
  }
   ...
   * endpoint.
   */
  static auto ipv4(asio::io_context& io_context, std::string const& host,
    std::string const& service) -> PointerType
  {
    ...
    LOG_F(ERROR, "({} / {}) did not resolve to a valid IPv4 endpoint", host,
      service);
    throw std::system_error { detail::make_error_code(INVALID_IPV4_ADDRESS) };
  }
   ...
   * endpoint.
   */
  static auto ipv6(asio::io_context& io_context, std::string const& host,
    std::string const& service) -> PointerType
  {
    try {
      const auto endpoints = ResolveEndpoint(io_context, host, service);
    ...
    LOG_F(ERROR, "({} / {}) did not resolve to a valid IPv6 endpoint", host,
      service);
  ...
  {
    auto on_completion = [this, callback](std::error_code const& failure,
    ...
    // CHECK(reception_buffer_.size() == SAFE_PAYLOAD_SIZE);
    socket_.async_receive_from(
      asio::buffer(reception_buffer_), current_message_sender_, on_completion);
  }
    ...
    TSendCallback const& callback)
  {
    if (message.size() > SAFE_PAYLOAD_SIZE) {
      ...
      auto on_completion
        = [callback, message_copy](
            std::error_code const& failure, std::size_t /* bytes_sent */) {
            if (failure) {
              LOG_F(ERROR, "{}", failure.message());
            }
            callback(failure);
          };

      socket_.async_send_to(
        asio::buffer(*message_copy), ConvertEndpoint(endpoint), on_completion);
    }
  }
   ...
   */
  static auto ResolveEndpoint(asio::io_context& io_context,
    ...
    -> std::vector<IpEndpoint>
  {
    ...
    try {
      ...
      for (auto const& result : results) {
        ...
        endpoints.push_back(ConvertEndpoint(result));
      }
      return endpoints;
    } catch (std::system_error const& failure) {
      LOG_F(ERROR, "{}", failure.what());
...
private:
   ...
   * @throw std::system_error if the resolution fails.
   */
  static auto CreateUnderlyingSocket(asio::io_context& io_context,
   ...
   */
  static auto ConvertEndpoint(UnderlyingEndpointType const& uep) -> EndpointType
  {
...
using AsyncUdpChannel = Channel<asio::ip::udp::socket>;
