...
private:
  StoreValueTask(KeyType const &key, DataType data, NetworkType &network,
    ...
    LOG_F(1, "{} create store value task for '{}'", this->Name(),
        key.ToHex());
  }
  ...
  static void TryToStoreValue(std::shared_ptr<StoreValueTask> task,
    ...
    LOG_F(1, "{} trying to find closer peer to store '{}' value",
    ...
    if (task->AllRequestsCompleted()) {
      LOG_F(1, "{} task completed -> notify caller", task->Name());
  ...
  static void SendFindPeerRequest(FindNodeRequestBody const &request,
    ...
    LOG_F(1, "{} sending find peer request to store '{}' to '{}'",
    ...
    auto on_error = [task, current_candidate](std::error_code const &) {
      LOG_F(1, "{} peer {} timed out on find peer request for store value",
  ...
  static void HandleFindPeerResponse(EndpointType const &sender,
    ...
    try {
      ...
      LOG_F(1, "{} failed to deserialize find peer response ({})",
          task->Name(), ex.what());
      task->MarkCandidateAsInvalid(header.source_id_);
    }
  ...
  static void SendStoreRequest(
      Node const &current_candidate, std::shared_ptr<StoreValueTask> task) {
    LOG_F(1, "{} send store request of '{}' to '{}'", task->Name(),
