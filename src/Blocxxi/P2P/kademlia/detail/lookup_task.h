...
protected:
  ...
  void MarkCandidateAsValid(const Node::IdType &candidate_id) {
    ...
    LOG_F(2, "{} candidate {} marked as STATE_RESPONDED", this->Name(),
        candidate_id.ToHex());
  }
  ...
  void MarkCandidateAsInvalid(const Node::IdType &candidate_id) {
    ...
    LOG_F(2, "{} candidate {} marked as STATE_TIMED_OUT", this->Name(),
        candidate_id.ToHex());
  }
  ...
  std::vector<Node> SelectUnContactedCandidates(std::size_t max_count) {
    ...
    LOG_F(2, "{} selected {} new fresh (not contacted) candidate",
        this->Name(), selection.size());
    for (auto &peer : selection) {
      LOG_F(2, " -> {}", peer.ToString());
    }
    return selection;
  }
  ...
  std::vector<Node> GetValidCandidates(std::size_t max_count) {
    ...
    LOG_F(2, "{} selected {} valid (responded) candidate", this->Name(),
        selection.size());
    for (auto &peer : selection) {
      LOG_F(2, " -> {}", peer.ToString());
    }
  ...
  [[nodiscard]] bool AllRequestsCompleted() const {
    LOG_F(1, "{} checking if all tasks completed, in-flight={}",
