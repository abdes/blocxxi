...
#include <Blocxxi/P2P/kademlia/response_dispatcher.h>
 ...
 */
template <typename TChannel, typename TMessageSerializer> class Network {
public:
  ...
  {
    LOG_F(1, "Creating Network DONE at '{}' and '{}'",
  ...
  ~Network() { LOG_F(1, "Destroy Network"); }
  //@}
  ...
  {
    CHECK_F(handler != nullptr, "message handler must not be null");
  ...
  {
    LOG_F(1, "SendConvRequest to {}", destination.ToString());
    ...
    LOG_F(2, "  message serialized");

    auto on_message_sent
      = [this, correlation_id, on_response_received, on_error, timeout](
          std::error_code const& failure) {
          // If the send fails, report to the caller.
          if (failure) {
            LOG_F(1, "  message send failed");
            on_error(failure);
          } else {
            LOG_F(2, "  message sent");
  ...
  void SendUniRequest(Request const& request, EndpointType const& destination)
  {
    ...
    SendMessage(message, destination, [](std::error_code const& failure) {
      if (failure) {
        LOG_F(1, "failed to send uni-request: {}", failure.message());
      }
    });
  }
    ...
    Response const& response, EndpointType const& destination)
  {
    ...
    SendMessage(message, destination, [](std::error_code const& failure) {
      if (failure) {
        LOG_F(1, "failed to send response: {}", failure.message());
      } else {
        LOG_F(1, "response sent");
      }
    });
  }
...
private:
  ...
  void ScheduleReceive(TChannel& chan)
  {
    chan.AsyncReceive([this, &chan](std::error_code const& failure,
                        IpEndpoint const& sender, BufferReader const& buffer) {
