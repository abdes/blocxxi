...
#endif
#if NOVA_CLANG_VERSION && NOVA_HAS_WARNING("-Wreserved-identifier")
...
public:
  ...
  using const_iterator = std::deque<Node>::const_iterator;
  using reverse_iterator = std::deque<Node>::reverse_iterator;
  using const_reverse_iterator = std::deque<Node>::const_reverse_iterator;
  ...
  auto begin() const noexcept -> const_iterator { return nodes_.cbegin(); }
  auto end() noexcept -> iterator { return nodes_.end(); }
  auto end() const noexcept -> const_iterator { return nodes_.cend(); }
  ...
  auto LeastRecentlySeenNode() -> Node& { return nodes_.front(); }
  /// Get the least recently seen node in the bucket.
  auto LeastRecentlySeenNode() const -> Node const& { return nodes_.front(); }
