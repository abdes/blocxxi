...
namespace {
  ...
  inline void SerializeInteger(IntegerType value, Buffer& b)
  {
    ...
    using unsigned_integer_type = std::make_unsigned_t<IntegerType>;

    LOG_F(1, "serialize integer {}", value);
    for (auto i = 0U; i < sizeof(value); ++i) {
      LOG_F(1, "  write one byte: {}", static_cast<Buffer::value_type>(value));
  ...
  inline auto Deserialize(
    BufferReader const& buffer, std::vector<std::uint8_t>& data) -> std::size_t
  {
    std::vector<std::uint8_t>::size_type size = 0;
    auto consumed = DeserializeInteger(buffer, size);
    const auto sub = buffer.subspan(consumed);
  ...
  {
    LOG_F(1, "serialize hash 160");
    b.insert(b.end(), i.begin(), i.end());
  }

  inline auto Deserialize(
    BufferReader const& buffer, blocxxi::crypto::Hash160& new_id) -> std::size_t
  {
    if (buffer.size_bytes() < blocxxi::crypto::Hash160::Size()) {
      throw std::range_error("insufficient data in input buffer");
    }
    LOG_F(1, "deserialize hash 160");
    std::copy_n(
      buffer.begin(), blocxxi::crypto::Hash160::Size(), new_id.begin());
    ...
    Header::MessageType& message_type) -> std::size_t
  {
    ...
    const auto first_byte = *std::cbegin(buffer);
    LOG_F(1, "deserialize header version/type {}", first_byte);
  ...
  inline void Serialize(asio::ip::address const& address, Buffer& b)
  {
    LOG_F(
      1, "serialize address {} is_v4={}", address.to_string(), address.is_v4());
    if (address.is_v4()) {
      b.push_back(KADEMLIA_ENDPOINT_SERIALIZATION_IPV4);
      auto const& a = address.to_v4().to_bytes();
      b.insert(b.end(), a.begin(), a.end());
    } else {
      // CHECK(address.is_v6() && "unknown IP version");
  ...
  inline auto Deserialize(
    BufferReader const& buffer, asio::ip::address& address) -> std::size_t
  {
    if (buffer.size_bytes() < 1) {
      throw std::range_error("insufficient data in input buffer");
    }
    ...
    LOG_F(1, "deserialize address protocol={}", protocol);
    if (protocol == KADEMLIA_ENDPOINT_SERIALIZATION_IPV4) {
      asio::ip::address_v4 a;
      consumed += DeserializeAddress(buffer.subspan(consumed), a);

      address = a;
    } else {
      CHECK_F((protocol == KADEMLIA_ENDPOINT_SERIALIZATION_IPV6),
        "unknown IP version");

      asio::ip::address_v6 a;
      consumed += DeserializeAddress(buffer.subspan(consumed), a);
  ...
  inline void Serialize(Node const& node, Buffer& buffer)
  {
    ...
    const std::uint16_t port = node.Endpoint().Port();
    SerializeInteger(port, buffer);
    Serialize(node.Endpoint().Address(), buffer);
  }
...
inline auto operator<<(
  std::ostream& out, Header::MessageType const& message_type) -> std::ostream&
{
  switch (message_type) {
...
inline auto operator<<(std::ostream& out, Header const& header) -> std::ostream&
{
...
{
  LOG_F(1, "serialize header version/type");
...
{
  const std::size_t size = body.peers_.size();
