...
// TODO(abdes): use defined(_POSIX_VERSION) for some of these things?
...
static auto basename_for_matching(const char* file_path) -> std::string;
static auto fullpath_for_matching(const char* file_path) -> std::string;
static auto compute_module_verbosity_for_paths(
  const std::string& base_name, const std::string& full_path) -> int;
...
static void ensure_flush_thread_started()
{
  ...
  s_flush_thread
    = std::make_unique<std::jthread>([](std::stop_token stop) -> void {
...
static const bool s_terminal_has_color = []() -> bool {
#  ifdef _WIN32
...
*/
static auto parse_vmodule_override(std::string_view override_str) -> bool
{
  ...
  auto trim = [](std::string_view& sv) -> void {
    while (
...
static auto match_segment(std::string_view patt, std::string_view seg) -> bool
{
  size_t pi = 0, si = 0;
...
static auto module_glob_match_no_double_star(
  std::string_view pattern, std::string_view path) -> bool
{
  size_t p_start = 0;
  size_t s_start = 0;
  while (true) {
...
static auto module_glob_match(
  const std::string& pattern, const std::string& path) -> bool
{
  if (pattern == path) {
    return true;
  }
...
LOGURU_EXPORT auto compute_module_verbosity_for(const char* file_path) -> int
{
...
void clear_current_thread_name();
#  endif
...
auto get_fatal_handler() -> fatal_handler_t { return s_fatal_handler; }
...
auto remove_callback(const char* id) -> bool
{
  ...
  auto it = std::find_if(begin(s_callbacks), end(s_callbacks),
    [&](const Callback& c) -> bool { return c.id == id; });
...
void clear_current_thread_name()
{
  ...
  if (void* current_name = pthread_getspecific(s_pthread_key_name)) {
    (void)pthread_setspecific(s_pthread_key_name, nullptr);
    free(current_name);
  }
}
#  endif
...
#  else // LOGURU_PTHREADS
  // TODO(abdes): on these weird platforms we should also store the thread name
...
auto ec_to_text(char c) -> Text
{
  ...
  auto write_hex_digit = [&](unsigned num) -> void {
    if (num < 10u) {
      str += char('0' + num);
    } else {
      str += char('a' + num - 10);
  ...
  auto write_hex_16 = [&](uint16_t n) -> void {
...
#  define DEFINE_EC(Type)                                                      \
    auto ec_to_text(Type value) -> Text                                        \
    {                                                                          \
