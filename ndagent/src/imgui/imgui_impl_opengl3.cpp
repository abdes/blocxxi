// ImGui Renderer for: OpenGL3 (modern OpenGL with shaders / programmatic
// pipeline) This needs to be used along with a Platform Binding (e.g. GLFW,
// SDL, Win32, custom..) (Note: We are using GL3W as a helper library to access
// OpenGL functions since there is no standard header to access modern OpenGL
// functions easily. Alternatives are GLEW, Glad, etc..)
...
//  2018-06-08: Misc: Extracted imgui_impl_opengl3.cpp/.h away from the old
//  combined GLFW/SDL+OpenGL3 examples. 2018-06-08: OpenGL: Use
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data) {
  ...
  // ABDES    GLint last_sampler; glGetIntegerv(GL_SAMPLER_BINDING,
  // &last_sampler);
  ...
  // ABDES    if (glBindSampler) glBindSampler(0, 0); // We use combined
  // texture/sampler state. Applications using GL 3.3 may set that otherwise.
  ...
  // ABDES    if (glBindSampler) glBindSampler(0, last_sampler);
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects() {
  ...
  const GLchar* fragment_shader_with_version[2] = {g_GlslVersion,
                                                   fragment_shader};
