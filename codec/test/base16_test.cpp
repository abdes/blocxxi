...
std::vector<EncodeTestParams> createValidValuesForEncodeTest() {
  ...
  addTestValue<4>(
      valid_values, {{0x11, 0x22, 0x33, 0x44}}, false, false, "11223344");
  ...
  addTestValue<4>(
      valid_values, {{0x11, 0x22, 0x33, 0x44}}, false, true, "11223344");
  ...
  addTestValue<4>(
      valid_values, {{0xA4, 0xB3, 0xC2, 0xD1}}, true, false, "1D2C3B4A");
  ...
  addTestValue<4>(
      valid_values, {{0xA4, 0xB3, 0x62, 0xD1}}, true, true, "1d263b4a");
...
INSTANTIATE_TEST_SUITE_P(
    Base16, CoDecTest, ::testing::ValuesIn(createValidValuesForEncodeTest()));
...
TEST_P(CoDecTest, DecodeCorrectness) {
  uint8_t buf[256]{0};
  Decode(std::string_view(hex_), gsl::span<uint8_t>(buf), reverse_);
...
TEST(Base16, DecodeOddSizeHexStringAborts) {
  uint8_t buf[1]{0};
  ASSERT_DEATH(
      Decode(std::string_view("F"), gsl::span<uint8_t>(buf), false), ".*");
  ASSERT_DEATH(
      Decode(std::string_view("FAB2244"), gsl::span<uint8_t>(buf), false),
      ".*");
}
...
TEST(Base16, DecodeInputStringWithInvalidHexThrowsDomainError) {
  uint8_t buf[5]{0};
  ASSERT_THROW(
      Decode(std::string_view("FE%@33"), gsl::span<uint8_t>(buf), false),
      std::domain_error);
  ASSERT_THROW(
      Decode(std::string_view("FEA-33"), gsl::span<uint8_t>(buf), false),
      std::domain_error);
}
